/* ================================================================
   PAGE-KASIR.JS — Kasir / Point of Sale
   ================================================================ */
  ...
  <div class="pos-layout">
    <div class="pos-products">
      <div style="display:flex;gap:10px;margin-bottom:12px;flex-wrap:wrap;">
        ...
        <select class="form-control" id="pos-kat" onchange="filterPOS()" style="width:auto;min-width:140px;">
          <option value="Semua">Semua Kategori</option>
          ${DB.kategori.map(k => `<option>${escapeHtml(k.nama)}</option>`).join('')}
        </select>
      </div>
      ...
      <div class="pos-cart-footer">
        <div style="display:flex;flex-direction:column;gap:6px;margin-bottom:10px;">
          ...
          <div style="display:flex;justify-content:space-between;align-items:center;">
            ...
            <input type="number" id="cart-diskon" class="form-control" style="width:110px;padding:4px 8px;font-size:12px;" value="0" min="0" oninput="updateCartTotal()" placeholder="0"/>
          </div>
        ...
        <div style="display:flex;flex-direction:column;gap:8px;">
          <select class="form-control" id="cart-metode">
            ${DB.metodePembayaran.filter(m => m.aktif).map(m => `<option>${escapeHtml(m.nama)}</option>`).join('')}
          </select>
...
/* ---------------------------------------------------------------
   PELANGGAN POS — pencarian + tambah cepat
   --------------------------------------------------------------- */
...
function cariPelangganPOS() {
  _posSelectedPelId = null; // reset pilihan saat user mengetik ulang
  const input = document.getElementById('cart-pelanggan-input');
  ...
  if (results.length === 0 && q) {
    ...
    dd.innerHTML = `<div style="padding:10px 14px;color:var(--text3);font-size:13px;"><i class="fa-solid fa-user-plus"></i> "${escapeHtml(input.value)}" — akan dipakai sebagai nama baru. Klik <i class="fa-solid fa-user-plus"></i> untuk simpan ke daftar.</div>`;
    return;
  }
  ...
  dd.innerHTML = results.map(p => `
    <div onclick="pilihPelangganPOS(${p.id})"
      ...
      <div>
        <div style="font-weight:600;color:var(--text);">${escapeHtml(p.nama)}</div>
        <div style="font-size:11px;color:var(--text3);">${escapeHtml(p.tlp || '—')} ${p.piutang > 0 ? '· <span style="color:var(--danger);">Piutang ' + fmtRp(p.piutang) + '</span>' : ''}</div>
      </div>
    </div>
  `).join('');
}

function pilihPelangganPOS(id) {
  const p = DB.pelanggan.find(x => x.id === id);
  if (!p) return;
  ...
  showToast('Pelanggan: ' + p.nama, 'success');
}
    ...
    <div class="modal-body">
      <div class="form-grid form-grid-2">
        <div class="form-group" style="grid-column:1/-1;">
          ...
          <input id="cp-nama" class="form-control" value="${escapeHtml(namaAwal)}" placeholder="Nama lengkap pelanggan" autofocus/>
        </div>
...
function filterPOS() {
  ...
  kasirKategori = document.getElementById('pos-kat')?.value || 'Semua';
  renderPOSGrid();
}
...
function renderPOSGrid() {
  ...
  grid.innerHTML = items.map(b => `
    <div class="product-card" onclick="addToCart(${b.id})">
      ${b.img
        ? `<img class="product-img" src="${escapeHtml(b.img)}" alt="${escapeHtml(b.nama)}" onerror="this.style.display='none';this.nextElementSibling.style.display='flex';" loading="lazy"/>
      ...
      <div class="product-name">${escapeHtml(b.nama)}</div>
      <div class="product-price">${fmtRp(b.hargaJual)}</div>
      <div style="font-size:10px;color:${b.stok <= b.stokMin ? 'var(--danger)' : 'var(--text3)'};">Stok: ${fmt(b.stok)} ${escapeHtml(b.satuan)}</div>
    </div>
...
function addToCart(id) {
  ...
  if (!b) return;
  if (b.stok <= 0) { showToast('Stok ' + b.nama + ' habis!', 'danger'); return; }
  const existing = appState.cart.find(c => c.id === id);
  if (existing) {
    if (existing.qty + 1 > b.stok) { showToast('Stok tidak cukup!', 'danger'); return; }
    existing.qty++;
  } else {
    appState.cart.push({ id: b.id, nama: b.nama, harga: b.hargaJual, qty: 1, emoji: b.emoji, img: b.img || '' });
  }
  renderCart();
...
function changeQty(id, delta) {
  ...
  if (!item) return;
  const b = DB.barang.find(x => x.id === id);
  if (delta > 0 && b && item.qty + delta > b.stok) { showToast('Stok tidak cukup!', 'danger'); return; }
  item.qty += delta;
...
function renderCart() {
  ...
  el.innerHTML = appState.cart.map(c => `
    <div class="pos-cart-item">
      ${c.img
        ? `<img class="product-img-sm" src="${escapeHtml(c.img)}" alt="${escapeHtml(c.nama)}" onerror="this.style.display='none';this.nextElementSibling.style.display='flex';" loading="lazy"/>
      ...
      <div style="flex:1;min-width:0;">
        <div style="font-size:12px;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${escapeHtml(c.nama)}</div>
...
/* ---------------------------------------------------------------
   PEMBAYARAN
   --------------------------------------------------------------- */
let _pendingTx = null; // simpan konteks transaksi sementara agar onclick tidak perlu string-encode data kompleks
...
function prosesTransaksi() {
  ...
  _pendingTx = { total, metode, pelId, namaDisplay };
  ...
  openModal(`
    ...
    <div class="modal-body">
      <div style="text-align:center;margin-bottom:16px;">
        ...
        <div style="color:var(--text2);">Metode: <strong>${escapeHtml(metode)}</strong></div>
        <div style="color:var(--text2);">Pelanggan: <strong>${escapeHtml(namaDisplay)}</strong></div>
      </div>
      ${metode === 'Tunai' ? `
      ...
      ` : '<div style="text-align:center;padding:12px;color:var(--text2);">Lakukan pembayaran via ' + escapeHtml(metode) + '</div>'}
    ...
    <div class="modal-footer">
      <button class="btn btn-outline" onclick="closeModal()">Batal</button>
      <button class="btn btn-success" onclick="bayarSelesai()"><i class="fa-solid fa-check-double"></i> Selesai & Cetak Struk</button>
    </div>
  `, 'modal-sm');
}
...
function bayarSelesai() {
  if (!_pendingTx) return;
  const { total, metode, pelId, namaDisplay } = _pendingTx;

  if (metode === 'Tunai') {
    const bayar = parseInt(document.getElementById('bayar-nominal')?.value) || 0;
    if (bayar < total) { showToast('Nominal bayar kurang dari total!', 'danger'); return; }
  }
  ...
  const nota = {
    ...
    noNota,
    tanggal: today(),
    pelanggan: pel ? pel.nama : (namaDisplay && namaDisplay !== 'Umum' ? namaDisplay : 'Umum'),
    pelangganId: pel ? pel.id : null,
    items, total, dp: total, sisa: 0, lunas: true,
    metode,
...
function simpanDP() {
  ...
  _pendingTx = { total, pelId, pelNamaDisplay };
  ...
  openModal(`
    ...
    <div class="modal-body">
      ...
      <div style="background:var(--primary-light);border-radius:9px;padding:10px 14px;margin-bottom:12px;font-size:13px;color:var(--primary);">
        <i class="fa-solid fa-user"></i> Pelanggan: <strong>${escapeHtml(pelNamaDisplay)}</strong>
      ...
      <div class="form-grid">
        <div class="form-group">
          ...
          <input type="number" id="dp-nominal" class="form-control" value="${Math.floor(total / 2)}" min="0" max="${total}" placeholder="Jumlah DP"/>
        </div>
        <div class="form-group">
    ...
    <div class="modal-footer">
      <button class="btn btn-outline" onclick="closeModal()">Batal</button>
      <button class="btn btn-warning" onclick="simpanDPSelesai()"><i class="fa-solid fa-floppy-disk"></i> Simpan DP</button>
    </div>
  `, 'modal-sm');
}

function simpanDPSelesai() {
  if (!_pendingTx) return;
  const { total, pelId, pelNamaDisplay } = _pendingTx;
  const dp = parseInt(document.getElementById('dp-nominal').value) || 0;
  if (dp <= 0 || dp > total) { showToast('Nominal DP tidak valid!', 'danger'); return; }
  ...
  const nota = {
    ...
    noNota,
    tanggal: today(),
    pelanggan: pel ? pel.nama : (pelNamaDisplay && pelNamaDisplay !== 'Umum' ? pelNamaDisplay : 'Umum'),
    pelangganId: pel ? pel.id : null,
    items, total, dp, sisa: total - dp, lunas: false,
    metode: 'Tunai (DP)',
    kasir: appState.user.nama,
    status: 'DP',
    statusOrder: 'Design',
...
/* ---------------------------------------------------------------
   STRUK / RECEIPT + WHATSAPP
   --------------------------------------------------------------- */
let _strukNotaCache = null; // simpan objek nota aktif agar tombol WA tidak perlu serialisasi ke string onclick

function showStruk(nota) {
  _strukNotaCache = nota;
    ...
    <div class="modal-body" style="padding-bottom:4px;">
      <div class="struk-preview" id="struk-content">
        <div class="struk-header">
          <div style="font-size:15px;font-weight:bold;">${escapeHtml(DB.pengaturanToko.nama)}</div>
          <div style="font-size:10px;">${escapeHtml(DB.pengaturanToko.alamat)}</div>
          <div style="font-size:10px;">Telp: ${escapeHtml(DB.pengaturanToko.tlp)}</div>
        </div>
        <div style="font-size:11px;margin:4px 0;">No: <strong>${nota.noNota}</strong></div>
        <div style="font-size:11px;">Tgl: ${tgl}</div>
        <div style="font-size:11px;">Kasir: ${escapeHtml(nota.kasir)}</div>
        <div style="font-size:11px;">Plgn: ${escapeHtml(nota.pelanggan)}</div>
        <div class="struk-divider"></div>
        ${nota.items.map(i => `
          <div class="struk-row"><span style="font-size:11px;flex:1;">${escapeHtml(i.nama)}</span></div>
        ...
        <div class="struk-row"><span>Metode</span><span>${escapeHtml(nota.metode)}</span></div>
        <div class="struk-row"><span>Status</span><span>${escapeHtml(nota.status)}</span></div>
        <div class="struk-divider"></div>
        <div class="struk-footer">
          <div>★ Terima Kasih ★</div>
          <div>${escapeHtml(DB.pengaturanToko.tagline)}</div>
        </div>
      </div>
      ...
      <div style="margin-top:14px;background:linear-gradient(135deg,#e7fbe9 0%,#d4f8d4 100%);border-radius:12px;padding:14px 16px;border:1.5px solid #b2dfb2;">
        ...
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;padding:8px 12px;background:#fff;border-radius:9px;border:1px solid #c3e6c3;cursor:pointer;" onclick="document.getElementById('wa-nomor').value='${nomorTersimpan}';highlightWAInput()">
          ...
          <div style="flex:1;">
            <div style="font-size:12px;font-weight:600;color:#1a7c1a;">${escapeHtml(pelData.nama)}</div>
            <div style="font-size:11px;color:#555;">${escapeHtml(pelData.tlp)}</div>
          </div>
        ...
        <div style="display:flex;gap:8px;align-items:center;">
          <div style="flex:1;position:relative;">
            ...
            <input id="wa-nomor" class="form-control" type="tel"
              ...
              onkeydown="if(event.key==='Enter')kirimWAStruk()"/>
          </div>
          <button class="btn" onclick="kirimWAStruk()"
...
function kirimWAStruk() {
  ...
  const tgl = new Date(nota.tanggal).toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' });
  const itemLines = nota.items.map(i => `  • ${i.nama}\n    ${i.qty} x ${fmtRp(i.harga)} = ${fmtRp(i.subtotal)}`).join('\n');
  const sisaLine = nota.sisa > 0 ? `\n💳 *Sisa Bayar :* ${fmtRp(nota.sisa)}` : '';

  const pesan =
`🖨️ *${DB.pengaturanToko.nama}*
${DB.pengaturanToko.alamat}
📞 ${DB.pengaturanToko.tlp}
─────────────────────
📋 *STRUK PEMBELIAN*
─────────────────────
No. Nota  : *${nota.noNota}*
...
${itemLines}
─────────────────────
💰 *Total     : ${fmtRp(nota.total)}*
✅ *DP / Bayar: ${fmtRp(nota.dp)}*${sisaLine}
💳 Metode    : ${nota.metode}
📌 Status    : ${nota.status}
─────────────────────
🙏 Terima kasih, ${nota.pelanggan}!
_${DB.pengaturanToko.tagline}_ ✨`;
