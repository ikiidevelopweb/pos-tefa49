/* ================================================================
   DATA.JS — Default state, seed data, & konfigurasi awal
   ================================================================ */

// ===== APP STATE (session, bukan data master) =====
let appState = {
  user: null,
...
// ===== DATABASE UTAMA (disimpan ke Firestore + localStorage) =====
let DB = {
  users: [
    { id: 1, username: 'admin',  password: 'tefa2424',    role: 'admin', nama: 'Ikii',    aktif: true, lastLogin: '' },
    { id: 2, username: 'admin2', password: 'admintefa49', role: 'admin', nama: 'Wahyono', aktif: true, lastLogin: '' },
  ...
  satuan: [
    { id: 1, nama: 'Lembar' }, { id: 2, nama: 'Pcs' }, { id: 3, nama: 'm²' },
    { id: 4, nama: 'Rim' }, { id: 5, nama: 'Box' }, { id: 6, nama: 'Set' },
  ],
  pelanggan: [
...
/**
 * Generate contoh transaksi (hanya dipanggil sekali, jika belum ada data tersimpan).
 * Dipisah jadi fungsi agar bisa dipanggil ulang dari sync.js setelah load selesai.
 */
function generateSampleData() {
  ...
  for (let i = 0; i < 15; i++) {
    ...
    DB.notaJual.push({
      ...
      pelanggan: cust.nama,
      pelangganId: cust.id,
      items, total, dp, sisa: total - dp, lunas,
      metode: metode.nama,
  ...
  for (let i = 0; i < 8; i++) {
    ...
    DB.notaBeli.push({
      id: 2000 + i, noNota: 'NB' + String(2000 + i), tanggal: d.toISOString().split('T')[0],
      supplier: sup.nama, barang: b.nama, qty, hargaBeli: b.hargaBeli, total,
    });
  }
}
