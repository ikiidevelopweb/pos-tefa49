...
function pageDashboard(){
  ...
  <div class="page-header">
    <div class="page-header-left">
      ...
      <p>${new Date().toLocaleDateString('id-ID',{weekday:'long',year:'numeric',month:'long',day:'numeric'})} · ${appState.user.nama} (${appState.user.role})</p>
    </div>
        ...
        <div class="chart-bar-container">
          ${chartData.map(c=>`
            <div class="chart-bar-wrap">
              <div style="font-size:10px;color:var(--text2);font-weight:600;">${c.val>0?fmtRp(c.val/1000)+'k':''}</div>
        ...
        stokRendah.slice(0,5).map(b=>`
          <div style="display:flex;align-items:center;gap:10px;padding:8px;border-radius:8px;background:var(--surface2);margin-bottom:6px;">
            ...
            <div style="flex:1;min-width:0;">
              <div style="font-size:12px;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${b.nama}</div>
              <div style="font-size:11px;color:var(--danger);">Stok: ${b.stok} ${b.satuan} (min: ${b.stokMin})</div>
            </div>
          ...
          <tbody>
            ${recentOrders.map(o=>`
              <tr>
                ...
                <td>${fmtRp(o.total)}</td>
                <td>${fmtRp(o.dp)}</td>
                <td><span class="badge badge-info">${o.metode}</span></td>
                <td><span class="badge ${o.lunas?'badge-success':'badge-warning'}">${o.status}</span></td>
                <td style="font-size:12px;">${o.kasir}</td>
              </tr>
            `).join('')}
          </tbody>
        </table>
