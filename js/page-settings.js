...
function pageLogLogin(){
  ...
  <div class="card"><div class="card-body" style="padding:0;"><div class="table-wrapper"><table>
    ...
    <tbody>${DB.logLogin.map((l,i)=>`<tr><td>${i+1}</td><td>${l.user}</td><td><span class="badge ${l.role==='admin'?'badge-purple':'badge-info'}">${l.role}</span></td><td>${l.waktu}</td><td>${l.ip}</td><td><span class="badge badge-success">${l.status}</span></td></tr>`).join('')||'<tr><td colspan="6" style="text-align:center;padding:30px;color:var(--text3);"><i class="fa-solid fa-inbox" style="font-size:24px;display:block;margin-bottom:8px;"></i>Belum ada riwayat login</td></tr>'}</tbody>
  </table></div></div></div>`;
}
...
function modalTambahUser(idx=-1){
  ...
  <div class="modal-body"><div class="form-grid form-grid-2">
    <div class="form-group"><label class="form-label">Nama Lengkap</label><input id="u-nama" class="form-control" value="${u.nama||''}"/></div>
    <div class="form-group"><label class="form-label">Username</label><input id="u-uname" class="form-control" value="${u.username||''}"/></div>
    <div class="form-group"><label class="form-label">Password</label><input type="password" id="u-pass" class="form-control" value="${u.password||''}" placeholder="Isi untuk ubah"/></div>
...
function simpanUser(idx){
  const obj={nama:document.getElementById('u-nama').value,username:document.getElementById('u-uname').value,password:document.getElementById('u-pass').value||DB.users[idx]?.password,role:document.getElementById('u-role').value,aktif:document.getElementById('u-aktif').value==='1'};
  if(idx>=0){Object.assign(DB.users[idx],obj);}else{obj.id=Date.now();DB.users.push(obj);}
  saveDB(); closeModal();showPage('pengaturan-user');showToast('User disimpan','success');
}
function hapusUser(i){if(confirm('Hapus user ini?')){DB.users.splice(i,1);saveDB();showPage('pengaturan-user');showToast('User dihapus','danger');}}
...
function pageProfilToko(){
  ...
  <div class="card" style="max-width:600px;">
    ...
    <div class="card-body">
      <div class="form-grid">
        <div class="form-group"><label class="form-label">Nama Toko</label><input id="tk-nama" class="form-control" value="${t.nama}"/></div>
        <div class="form-group"><label class="form-label">Tagline</label><input id="tk-tag" class="form-control" value="${t.tagline}"/></div>
        <div class="form-group"><label class="form-label">Alamat</label><input id="tk-almt" class="form-control" value="${t.alamat}"/></div>
        <div class="form-group"><label class="form-label">Telepon / WA</label><input id="tk-tlp" class="form-control" value="${t.tlp}"/></div>
        <div class="form-group"><label class="form-label">Email</label><input id="tk-email" class="form-control" value="${t.email}"/></div>
        <div class="form-group"><label class="form-label">Logo (Emoji)</label><input id="tk-logo" class="form-control" value="${t.logo}"/></div>
      </div>
  ...
  <div class="card" style="max-width:600px;margin-top:16px;border-color:#fca5a5;">
    ...
    <div class="card-body">
      <p style="font-size:13px;color:var(--text2);margin-bottom:14px;">Data tersimpan otomatis di browser <b>dan disinkronkan ke cloud</b> agar bisa diakses dari device lain. Klik ikon sync di topbar untuk memperbarui manual.</p>
...
function simpanToko(){
  DB.pengaturanToko.nama=document.getElementById('tk-nama').value;
  DB.pengaturanToko.tagline=document.getElementById('tk-tag').value;
  DB.pengaturanToko.alamat=document.getElementById('tk-almt').value;
  DB.pengaturanToko.tlp=document.getElementById('tk-tlp').value;
  DB.pengaturanToko.email=document.getElementById('tk-email').value;
  DB.pengaturanToko.logo=document.getElementById('tk-logo').value;
  saveDB();
...
function pageProfil(){
  ...
  <div class="card" style="max-width:500px;">
    <div class="card-body" style="text-align:center;padding:30px;">
      ...
      <h2 style="font-size:20px;font-weight:800;">${u.nama}</h2>
      <p style="color:var(--text2);margin-bottom:4px;">${u.username}</p>
    ...
    <div style="border-top:1.5px solid var(--border);padding:20px;">
      <div class="form-grid">
        <div class="form-group"><label class="form-label">Nama Lengkap</label><input id="pf-nama" class="form-control" value="${u.nama}"/></div>
        <div class="form-group"><label class="form-label">Username</label><input id="pf-uname" class="form-control" value="${u.username}"/></div>
...
function simpanProfil(){
  const u=appState.user;
  u.nama=document.getElementById('pf-nama').value;
  u.username=document.getElementById('pf-uname').value;
  ...
  document.getElementById('sb-avatar').textContent=u.nama[0];
...
async function importBackupFile(input){
  ...
  reader.onload = async (e) => {
    ...
    } catch(err) { showToast('Gagal membaca file backup!', 'danger'); }
  };
