    ...
    <div class="card-body" style="padding:0;"><div class="table-wrapper"><table>
      ...
      <tbody>${DB.notaJual.map((n,i)=>`
        <tr>
          ...
          <td><span class="badge badge-info">${n.metode}</span></td>
          <td><span class="badge ${n.lunas?'badge-success':'badge-warning'}">${n.status}</span></td>
          <td style="font-size:11px;">${n.kasir}</td>
          <td><span class="badge badge-purple">${n.statusOrder}</span></td>
...
function renderLaporanJualBarang(){
  ...
  <div class="card-body" style="padding:0;"><div class="table-wrapper"><table>
    ...
    <tbody>${rows.map((r,i)=>`<tr><td>${i+1}</td><td>${r.nama}</td><td>${fmt(r.qty)}</td><td>${fmtRp(r.total)}</td></tr>`).join('')}</tbody>
  </table></div></div></div>`;
}
...
function renderLaporanNotaBeli(){
  ...
  <div class="card-body" style="padding:0;"><div class="table-wrapper"><table>
    ...
    <tbody>${DB.notaBeli.map(n=>`<tr><td><strong>${n.noNota}</strong></td><td>${n.tanggal}</td><td>${n.supplier}</td><td>${n.barang}</td><td>${fmt(n.qty)}</td><td>${fmtRp(n.total)}</td></tr>`).join('')}</tbody>
  </table></div></div></div>`;
}
