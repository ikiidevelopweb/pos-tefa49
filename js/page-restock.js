      ...
      <div class="table-wrapper">
        <table>
          ...
          <tbody>
            ${stokRendah.map(b=>`
              <tr>
                <td><i class="${b.emoji||'fa-solid fa-box'}"></i> ${b.nama}</td>
                <td><span class="badge badge-danger">${b.stok} ${b.satuan}</span></td>
                <td>${b.stokMin}</td>
                <td>${Math.max(0, b.stokMin-b.stok)}</td>
      ...
      <div class="form-grid form-grid-2">
        <div class="form-group">
          ...
          <select id="rst-supplier" class="form-control">
            ${DB.supplier.map(s=>`<option>${s.nama}</option>`).join('')}
          </select>
        </div>
        <div class="form-group">
          <label class="form-label">Barang</label>
          <select id="rst-barang" class="form-control">
            ${DB.barang.map(b=>`<option value="${b.id}">${b.nama}</option>`).join('')}
          </select>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
          ...
          <input type="number" id="rst-harga" class="form-control" value="200"/>
        </div>
...
function modalRestockBarang(id){
  ...
  openModal(`
    <div class="modal-header"><span class="modal-title"><i class="fa-solid fa-box-open"></i> Restock: ${b.nama}</span><button class="modal-close" onclick="closeModal()">×</button></div>
    <div class="modal-body">
      <p>Stok saat ini: <strong>${b.stok} ${b.satuan}</strong> (Min: ${b.stokMin})</p>
      <div class="form-grid" style="margin-top:12px;">
        <div class="form-group"><label class="form-label">Qty Tambah</label><input type="number" id="rst-qty-q" class="form-control" value="${b.stokMin*2}"/></div>
        <div class="form-group"><label class="form-label">Harga Beli</label><input type="number" id="rst-harga-q" class="form-control" value="${b.hargaBeli}"/></div>
        <div class="form-group"><label class="form-label">Supplier</label><select id="rst-sup-q" class="form-control">${DB.supplier.map(s=>`<option>${s.nama}</option>`).join('')}</select></div>
      </div>
    </div>
    <div class="modal-footer">
      <button class="btn btn-outline" onclick="closeModal()">Batal</button>
